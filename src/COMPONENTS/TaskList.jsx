...
export default function TaskList({ tasks, onDelete, editTask, onFav }) {
  return (
    <>
          ...
          <tbody>
            {tasks.map((task) => (
              <tr key={task.id} className="border-b border-[#2E3443] [&>td]:align-baseline [&>td]:px-4 [&>td]:py-2">
                <td>
                  <button onClick={() => onFav(task.id)}>{task.isFavorite ? <FaStar color="yellow" /> : <FaStar color="gray" />}</button>
                </td>
                <td>{task.title}</td>
                <td>
